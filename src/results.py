...
def mostrar_resultados(
    ...
    """Ejecuta CCR y BCC, genera tablas y figuras de Plotly y devuelve todos los resultados."""
    resultados = {}
    ...
    # 3) Renombrar columnas de eficiencia para los histogramas
    df_ccr_for_plots = df_ccr.rename(columns={"tec_efficiency_ccr": "efficiency"}) \
        if df_ccr is not None else pd.DataFrame()

    resultados["df_ccr"] = df_ccr
    resultados["df_bcc"] = df_bcc

    # 4) Unir resultados con datos originales para el spider plot
