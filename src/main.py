...
# --- 3) FUNCIONES DE LÓGICA DE IA (SIMULADAS) ---
@st.cache_data
...
class AppRenderer:
    ...
    def render_upload_step(self):
        ...
        if source_option == 'Usar caso de estudio':
            ...
            try:
                ...
                if not datasets:
                    st.warning("Carpeta `datasets` vacía o no encontrada en `src`."); return
    ...
    def render_preliminary_analysis_step(self, scenario):
        ...
        st.dataframe(scenario['df'].describe())
    ...
    def render_proposal_step(self, scenario):
        ...
        if choice == "Configuración Manual":
            inputs = st.multiselect("Inputs:", options=cols)
            outputs = st.multiselect("Outputs:", options=[c for c in cols if c not in inputs])
        else:
    ...
    def render_deliberation_workshop(self, scenario):
        ...
        if st.button("Generar Mapa Metodológico con IA", key=f"gen_map_{scenario['name']}"):
            with st.spinner("IA generando árbol de preguntas..."):
                ...
                tree, error = cached_run_inquiry_engine("Generar un árbol de auditoría para este análisis DEA", context)
        ...
        if scenario.get("inquiry_tree"):
            ...
            self.render_eee_explanation(eee_metrics)
            self.render_interactive_inquiry_tree(scenario)
    
    def render_eee_explanation(self, eee_metrics: dict):
        with st.expander("Ver desglose y justificación del EEE"):
            st.markdown(f"**Profundidad:** {eee_metrics.get('D1', 0):.2%} | **Pluralidad:** {eee_metrics.get('D2', 0):.2%} | **Robustez:** {eee_metrics.get('D5', 0):.2%}")
    
    def render_interactive_inquiry_tree(self, scenario):
        st.subheader("Justifica tu Razonamiento")
        tree = scenario.get("inquiry_tree", {})
        # ... (Aquí iría la lógica completa para renderizar el árbol interactivamente) ...
...
def main():
    ...
    # No es necesario llamar al navegador de escenarios aquí, se maneja en el flujo
    active_scenario = get_active_scenario()
