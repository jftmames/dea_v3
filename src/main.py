...
if script_dir not in sys.path: # ¡CORRECCIÓN AQUÍ!
    sys.path.insert(0, script_dir) # sys.path en lugar de sys.sys.path
...
def generate_analysis_proposals(df_columns: list[str], df_head: pd.DataFrame):
    ...
    try:
        ...
        if isinstance(resp, dict) and resp.get("error"): # Verifica si chat_completion devolvió un error
    ...
    if st.button(f"Ejecutar Análisis DEA para '{active_scenario['name']}'", type="primary", use_container_width=True, key=f"run_{st.session_state.active_scenario_id}"):
        with st.spinner(f"Ejecutando {model_name} para '{active_scenario['name']}'..."):
            ...
            if validation_results['formal_issues']:
                ...
                return # Detener la ejecución si hay errores for
