...
# Todas las funciones de renderizado de la UI se agrupan en esta clase.
# Esto asegura que todas estén definidas y accesibles como métodos de una instancia.
class AppRenderer:
    def __init__(self):
        # No se necesita pasar muchas dependencias si ya están importadas globalmente
        pass
    ...
    def render_comparison_view(self):
        ...
        for sc, col in [(scenario_a, res_col1), (scenario_b, res_col2)]:
            with col:
                st.subheader(f"Results of: {sc['name']}")
    ...
    def render_eee_explanation(self, eee_metrics: dict):
    ...
    def render_deliberation_workshop(self, active_scenario): 
        ...
        with st.container(border=True):
            ...
            if active_scenario.get("inquiry_tree"):
                ...
                self.render_eee_explanation(eee_metrics) # Call method of this class

        st.divider()
        
        self.render_interactive_inquiry_tree(active_scenario) # Call method of this class

    def render_optimization_workshop(self, active_scenario): 
        if not active_scenario.get('dea_results'): 
    ...
    def render_main_dashboard(self, active_scenario): 
        ...
        if st.button(f"Ejecutar Análisis DEA para '{active_scenario['name']}'", type="primary", use_container_width=True, help="Inicia el cálculo del modelo DEA con la configuración actual. Los resultados y las visualizaciones aparecerán a continuación."):
            with st.spinner(f"Ejecutando {model_name} para '{active_scenario['name']}'... Esto puede tomar unos segundos o minutos dependiendo del tamaño de tus datos."):
                ...
                validation_results = validate_data(df, proposal.get('inputs', []), proposal.get('outputs', [])) # Call global function
                ...
                try:
                    results = cached_run_dea_analysis(df, df.columns[0], proposal.get('inputs', []), proposal.get('outputs', []), model_key, period_col) # Call global cached function
        ...
        if active_scenario.get("dea_results"):
            ...
            self.render_optimization_workshop(active_scenario)
...
def main():
    ...
    st.sidebar.image("src/assets/DEA_logo.png", width=200)
    ...
    renderer.render_scenario_navigator() # Call method on instance
    ...
    with analysis_tab:
        ...
        # Process scenario creation/cloning requests from sidebar
        if '_new_scenario_requested' in st.session_state and st.session_state._new_scenario_requested:
            ...
            del st.session_state._new_scenario_requested # Clear flag
            st.rerun()
        if '_clone_scenario_requested' in st.session_state and st.session_state._clone_scenario_requested:
            ...
            del st.session_state._clone_scenario_requested # Clear flag
            st.rerun()

        # Render step based on app_status
